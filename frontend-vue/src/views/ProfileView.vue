<template>
  <div class="container profile-container">
    <div class="profile-layout">

      <!-- Sidebar -->
      <div class="profile-sidebar">
        <div class="avatar-section">
          <div class="avatar-big">{{ userInitial }}</div>
          <div class="avatar-name">{{ profile.username }}</div>
          <div class="avatar-role">Utilizator</div>
        </div>
        <div class="sidebar-links">
          <router-link to="/completed-builds" class="sidebar-link">📂 Build-urile mele</router-link>
          <span class="sidebar-link active">⚙️ Preferințele mele</span>
        </div>
      </div>

      <!-- Main content -->
      <div class="profile-main">
        <div class="profile-header">
          <h2>⚙️ Preferințele mele</h2>
          <p>Acestea vor fi folosite de agentul AI pentru a genera build-uri personalizate.</p>
        </div>

        <div v-if="loadingProfile" class="loading">Se încarcă profilul...</div>

        <div v-else class="profile-form">

          <!-- Buget -->
          <div class="form-section">
            <h4>💰 Buget</h4>
            <div class="form-row">
              <label>Buget maxim (RON)</label>
              <input
                type="number"
                v-model="form.buget_max"
                placeholder="ex: 5000"
                min="0"
              />
            </div>
          </div>

          <!-- Preferințe hardware -->
          <div class="form-section">
            <h4>🖥️ Preferințe Hardware</h4>

            <div class="form-row">
              <label>Procesor preferat (CPU)</label>
              <div class="select-group">
                <button
                  v-for="opt in cpuOptions" :key="opt.value"
                  class="select-btn"
                  :class="{ active: form.cpu_preferat === opt.value }"
                  @click="form.cpu_preferat = opt.value"
                >
                  {{ opt.label }}
                </button>
              </div>
            </div>

            <div class="form-row">
              <label>Placă video preferată (GPU)</label>
              <div class="select-group">
                <button
                  v-for="opt in gpuOptions" :key="opt.value"
                  class="select-btn"
                  :class="{ active: form.gpu_preferat === opt.value }"
                  @click="form.gpu_preferat = opt.value"
                >
                  {{ opt.label }}
                </button>
              </div>
            </div>

            <div class="form-row">
              <label>Sursă preferată (PSU)</label>
              <select v-model="form.psu_preferat" class="form-select">
                <option value="Oricare">Oricare (cel mai ieftin)</option>
                <optgroup label="Branduri Recomandate">
                  <option value="Seasonic">Seasonic</option>
                  <option value="Corsair">Corsair</option>
                  <option value="be quiet!">be quiet!</option>
                  <option value="ASUS">ASUS</option>
                  <option value="MSI">MSI</option>
                </optgroup>
                <optgroup label="Alte Branduri">
                  <option value="EVGA">EVGA</option>
                  <option value="FSP">FSP (Fortron)</option>
                  <option value="Gigabyte">Gigabyte</option>
                  <option value="Thermaltake">Thermaltake</option>
                  <option value="Deepcool">Deepcool</option>
                </optgroup>
              </select>
            </div>
          </div>

          <!-- Display & Locație -->
          <div class="form-section">
            <h4>🖥️ Display & Locație</h4>

            <div class="form-row">
              <label>Rezoluție dorită</label>
              <div class="select-group">
                <button
                  v-for="opt in rezolutieOptions" :key="opt.value"
                  class="select-btn"
                  :class="{ active: form.rezolutie_dorita === opt.value }"
                  @click="form.rezolutie_dorita = opt.value"
                >
                  {{ opt.label }}
                </button>
              </div>
            </div>

            <div class="form-row">
              <label>Locație</label>
              <input type="text" v-model="form.locatie" placeholder="ex: Romania" />
            </div>
          </div>

          <!-- Jocuri preferate -->
          <div class="form-section">
            <h4>🎮 Jocuri preferate</h4>
            <p class="section-hint">Adaugă jocurile pe care le joci — agentul va optimiza build-ul pentru ele.</p>
            <div class="jocuri-input-row">
              <input
                type="text"
                v-model="jocNou"
                placeholder="ex: Cyberpunk 2077"
                @keydown.enter.prevent="adaugaJoc"
              />
              <button class="btn-add-joc" @click="adaugaJoc">+ Adaugă</button>
            </div>
            <div class="jocuri-tags">
              <span v-for="(joc, i) in form.jocuri_pref" :key="i" class="joc-tag">
                {{ joc }}
                <button @click="stergeJoc(i)">✕</button>
              </span>
              <span v-if="form.jocuri_pref.length === 0" class="no-jocuri">Niciun joc adăugat</span>
            </div>
          </div>

          <!-- Save button -->
          <div class="form-actions">
            <button class="btn-save" :disabled="saving" @click="salveazaProfil">
              {{ saving ? '⏳ Se salvează...' : '💾 Salvează Preferințele' }}
            </button>
            <span v-if="saveSuccess" class="save-success">✅ Salvat cu succes!</span>
            <span v-if="saveError" class="save-error">❌ {{ saveError }}</span>
          </div>

        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { ref, computed, onMounted } from 'vue'
import api from '../plugins/axios'
import { useRouter } from 'vue-router'

const router = useRouter()
const loadingProfile = ref(true)
const saving = ref(false)
const saveSuccess = ref(false)
const saveError = ref('')
const jocNou = ref('')

const profile = ref({ username: localStorage.getItem('username') || 'Cont' })
const userInitial = computed(() => profile.value.username?.charAt(0).toUpperCase() || 'U')

const form = ref({
  buget_max: 0,
  cpu_preferat: 'Oricare',
  gpu_preferat: 'Oricare',
  psu_preferat: 'Oricare',
  rezolutie_dorita: '1080p',
  locatie: 'Romania',
  jocuri_pref: [],
})

const cpuOptions = [
  { value: 'Oricare', label: 'Oricare' },
  { value: 'AMD', label: 'AMD' },
  { value: 'Intel', label: 'Intel' },
]

const gpuOptions = [
  { value: 'Oricare', label: 'Oricare' },
  { value: 'NVIDIA', label: 'NVIDIA' },
  { value: 'AMD', label: 'AMD' },
  { value: 'Intel', label: 'Intel Arc' },
]

const rezolutieOptions = [
  { value: '1080p', label: 'Full HD 1080p' },
  { value: '1440p', label: 'Quad HD 1440p' },
  { value: '4K', label: 'Ultra HD 4K' },
]

const fetchProfile = async () => {
  const token = localStorage.getItem('access_token')
  if (!token) { router.push('/login'); return }

  try {
    const res = await api.get('accounts/profile/')
    profile.value = res.data
    form.value = {
      buget_max: res.data.buget_max || 0,
      cpu_preferat: res.data.cpu_preferat || 'Oricare',
      gpu_preferat: res.data.gpu_preferat || 'Oricare',
      psu_preferat: res.data.psu_preferat || 'Oricare',
      rezolutie_dorita: res.data.rezolutie_dorita || '1080p',
      locatie: res.data.locatie || 'Romania',
      jocuri_pref: res.data.jocuri_pref || [],
    }
  } catch (err) {
    console.error('Eroare la fetch profil:', err)
  } finally {
    loadingProfile.value = false
  }
}

const salveazaProfil = async () => {
  saving.value = true
  saveSuccess.value = false
  saveError.value = ''
  try {
    await api.put('accounts/profile/', form.value)
    saveSuccess.value = true
    setTimeout(() => saveSuccess.value = false, 3000)
  } catch (err) {
    saveError.value = 'Eroare la salvare. Încearcă din nou.'
    console.error(err)
  } finally {
    saving.value = false
  }
}

const adaugaJoc = () => {
  const joc = jocNou.value.trim()
  if (joc && !form.value.jocuri_pref.includes(joc)) {
    form.value.jocuri_pref.push(joc)
  }
  jocNou.value = ''
}

const stergeJoc = (index) => {
  form.value.jocuri_pref.splice(index, 1)
}

onMounted(fetchProfile)
</script>

<style scoped>
.profile-container { padding: 40px 20px; color: white; }

.profile-layout {
  display: grid;
  grid-template-columns: 220px 1fr;
  gap: 30px;
  max-width: 900px;
  margin: 0 auto;
}

/* Sidebar */
.profile-sidebar {
  background: #1a1b26;
  border: 1px solid #2a2d3e;
  border-radius: 12px;
  padding: 24px;
  height: fit-content;
}

.avatar-section { text-align: center; margin-bottom: 24px; }

.avatar-big {
  width: 60px; height: 60px;
  border-radius: 50%;
  background: #3b82f6;
  color: white;
  display: flex; align-items: center; justify-content: center;
  font-size: 1.6rem; font-weight: 700;
  margin: 0 auto 10px;
}

.avatar-name { color: white; font-weight: 600; font-size: 1rem; }
.avatar-role { color: #64748b; font-size: 0.8rem; margin-top: 2px; }

.sidebar-links { display: flex; flex-direction: column; gap: 4px; }

.sidebar-link {
  display: block;
  padding: 9px 12px;
  border-radius: 7px;
  color: #a9b1d6;
  font-size: 0.85rem;
  text-decoration: none;
  cursor: pointer;
  transition: 0.2s;
  border: none;
  background: none;
  text-align: left;
}
.sidebar-link:hover { background: #232533; color: white; }
.sidebar-link.active { background: #232533; color: #3b82f6; font-weight: 600; }

/* Main */
.profile-main {
  background: #1a1b26;
  border: 1px solid #2a2d3e;
  border-radius: 12px;
  padding: 28px;
}

.profile-header { margin-bottom: 28px; }
.profile-header h2 { font-size: 1.3rem; margin-bottom: 6px; }
.profile-header p { color: #64748b; font-size: 0.85rem; }

/* Sections */
.form-section {
  margin-bottom: 28px;
  padding-bottom: 28px;
  border-bottom: 1px solid #2a2d3e;
}
.form-section:last-of-type { border-bottom: none; }
.form-section h4 { color: #a9b1d6; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 16px; }

.section-hint { color: #64748b; font-size: 0.8rem; margin-bottom: 12px; margin-top: -10px; }

.form-row { margin-bottom: 16px; }
.form-row label { display: block; color: #e2e8f0; font-size: 0.85rem; margin-bottom: 8px; }

input[type="text"],
input[type="number"] {
  width: 100%;
  background: #0f111a;
  border: 1px solid #2a2d3e;
  border-radius: 7px;
  padding: 10px 14px;
  color: white;
  font-size: 0.9rem;
  outline: none;
  transition: border-color 0.2s;
}
input:focus { border-color: #3b82f6; }

.form-select {
  width: 100%;
  background: #0f111a;
  border: 1px solid #2a2d3e;
  border-radius: 7px;
  padding: 10px 14px;
  color: white;
  font-size: 0.9rem;
  outline: none;
  cursor: pointer;
}

/* Select buttons */
.select-group { display: flex; gap: 8px; flex-wrap: wrap; }

.select-btn {
  padding: 7px 16px;
  border-radius: 20px;
  border: 1px solid #2a2d3e;
  background: transparent;
  color: #a9b1d6;
  font-size: 0.85rem;
  cursor: pointer;
  transition: 0.2s;
}
.select-btn:hover { border-color: #3b82f6; color: white; }
.select-btn.active { background: #3b82f6; border-color: #3b82f6; color: white; font-weight: 600; }

/* Jocuri */
.jocuri-input-row { display: flex; gap: 8px; margin-bottom: 12px; }
.jocuri-input-row input { flex: 1; }

.btn-add-joc {
  background: #3b82f6;
  color: white;
  border: none;
  padding: 10px 16px;
  border-radius: 7px;
  cursor: pointer;
  font-weight: 600;
  white-space: nowrap;
  transition: 0.2s;
}
.btn-add-joc:hover { background: #2563eb; }

.jocuri-tags { display: flex; flex-wrap: wrap; gap: 8px; }

.joc-tag {
  display: flex; align-items: center; gap: 6px;
  background: #232533;
  border: 1px solid #2a2d3e;
  border-radius: 20px;
  padding: 4px 12px;
  font-size: 0.82rem;
  color: #c0caf5;
}
.joc-tag button {
  background: none; border: none; color: #64748b;
  cursor: pointer; font-size: 0.75rem; padding: 0;
  transition: color 0.2s;
}
.joc-tag button:hover { color: #f43f5e; }

.no-jocuri { color: #475569; font-size: 0.82rem; }

/* Actions */
.form-actions { display: flex; align-items: center; gap: 14px; margin-top: 8px; }

.btn-save {
  background: #10b981;
  color: white;
  border: none;
  padding: 12px 28px;
  border-radius: 8px;
  font-weight: 700;
  font-size: 0.95rem;
  cursor: pointer;
  transition: 0.2s;
}
.btn-save:hover:not(:disabled) { background: #059669; transform: translateY(-1px); }
.btn-save:disabled { opacity: 0.6; cursor: not-allowed; }

.save-success { color: #10b981; font-size: 0.85rem; }
.save-error { color: #f43f5e; font-size: 0.85rem; }

.loading { padding: 40px; text-align: center; color: #64748b; }

@media (max-width: 640px) {
  .profile-layout { grid-template-columns: 1fr; }
}
</style>